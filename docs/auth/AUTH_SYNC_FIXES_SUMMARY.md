...
See `AUTH_STATE_MACHINE_DIAGRAM.md` for the current Mermaid state machine covering the existing auth components:

- `App.js`, `Header.js`, `Register.js`, `Login.js`, `Profile.js`
- `authService.js`, `api.js`
- `authRoutes.js`, `validationMiddleware.js`, `authMiddleware.js`
- `authController.js`, `userModel.js`, and the `users` table
